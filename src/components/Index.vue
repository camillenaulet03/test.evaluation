    ...
    methods: {
      async updateMarker() {
        this.map.on("click", async (e) => {
          await axios.patch('http://localhost:3001/markers/'+this.id, {long: e.lngLat.lng, lat: e.lngLat.lat});
          window.location.reload();
        });
      },
      create() {
        this.map.on("click", async (e) => {
          await axios.post('http://localhost:3001/markers', {long: e.lngLat.lng, lat: e.lngLat.lat});
          window.location.reload();
        });
      },
      async remove() {
        await axios.delete('http://localhost:3001/markers/'+this.id);
        window.location.reload();
      }
    }
